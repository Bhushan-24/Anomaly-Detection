...
class Feature_extractor(object):
   def__init__(self,originpics,forgpics, ab_forgpics, U,V, weight):
   ...
   def getPosition(self,pics,index,style=True,mode=True):
       ...
       if mode:
          ...
          if style:
             ...
             im_s,mopho_img=poscalNormal(img,ad_img)
   ...
   def simple_feature(self,oics,index,style=True,mode=True):
       realPos,_, label,_=self.getPosition(pics,index,style,mode)
       feature=getFeatureUV(realPo,self.U[:,:,index]*np.sqrt(self.weigh).reshape((self.m,1),self.V[:,:,index]*np.sqrt(self.weigh)
       return feature,label

   def get_features_and_labels(self,start,end, mode=True):
       datal=np.zeros((0,2))
       data1Ab=np.zeros(0,2))
       label=np.zeros(0)
       labelAb=np.zeros(0)
       if mode:
              for i in range(start,end):
                  data,labe=self.simple_feature(self.forgpics,i)
                  dataAb,labeAb=self.simple_feature(self.ab_forgics,i,False)
                  data1=np.concatenate((data1,data),axis=0)
                  data1Ab=np.concatenate((data1Ab,dataAb),axis=0)
                  label=np.concatenate((label,labe),axis=0)
                  labelAb=np.concatenate((labelAb,labeAb),axis=0)
               features=np.nan_to_num(np.concatenate((data1,data1Ab),axis=0))     
               labels=np.nan_to_num(np.concatenate((label,labelAb),axis=0))
       else:
          features=np.zeros(0,2))
          for i in  range(start,end)
              data,_=self.simple_feature(self.forgpics,i,True,mode)
